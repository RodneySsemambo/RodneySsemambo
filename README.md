# Hi, I'm Rodney Ssemambo 👋

**Laravel & PHP Developer** based in Kampala, Uganda 🇺🇬

I build full-stack web applications and restfull APIs with clean architecture, proper testing, and real-world features like payment integrations and real-time chat.

---

## 🚀 What I'm Working On

- **[Doctors Booking App](https://github.com/RodneySsemambo/Doctors_Booking_App)** — A full-stack healthcare platform with appointment scheduling, MTN/Airtel mobile money payments, an AI-powered chatbot, and a real-time Livewire UI. Built with Laravel 11, Filament admin, and 31 passing tests.

---

## 🛠️ Tech Stack

**Backend**
![PHP](https://img.shields.io/badge/PHP-8.2-777BB4?style=flat&logo=php&logoColor=white)
![Laravel](https://img.shields.io/badge/Laravel-11-FF2D20?style=flat&logo=laravel&logoColor=white)
![MySQL](https://img.shields.io/badge/MySQL-8.0-4479A1?style=flat&logo=mysql&logoColor=white)

**Frontend**
![Livewire](https://img.shields.io/badge/Livewire-3-FB70A9?style=flat&logo=livewire&logoColor=white)
![Tailwind CSS](https://img.shields.io/badge/Tailwind-3-06B6D4?style=flat&logo=tailwindcss&logoColor=white)
![Alpine.js](https://img.shields.io/badge/Alpine.js-3-8BC0D0?style=flat&logo=alpinedotjs&logoColor=white)

**Tools & DevOps**
![Docker](https://img.shields.io/badge/Docker-2496ED?style=flat&logo=docker&logoColor=white)
![Git](https://img.shields.io/badge/Git-F05032?style=flat&logo=git&logoColor=white)
![PHPUnit](https://img.shields.io/badge/PHPUnit-Testing-366488?style=flat&logo=php&logoColor=white)
![Filament](https://img.shields.io/badge/Filament-Admin-FDAE4B?style=flat&logo=laravel&logoColor=white)

---

## 📌 Featured Projects

### 🏥 Doctors Booking App
> A production-ready healthcare booking platform

- Patient & doctor dashboards with role-based access
- Appointment scheduling with slot conflict prevention
- **MTN Mobile Money & Airtel Money** payment integration
- AI-powered chatbot for doctor discovery and booking
- Real-time UI with Laravel Livewire
- Filament admin panel for full system management
- **31 passing tests** covering payments, appointments, and chat

**Stack:** Laravel 11 · Livewire · Filament · MySQL · Docker · PHPUnit

[View Repository](https://github.com/RodneySsemambo/Doctors_Booking_App) · [Live Demo](#) ← add your Render URL here

---

## 📫 Get In Touch

[![Email](https://img.shields.io/badge/Email-ssemamborodney94@gmail.com-D14836?style=flat&logo=gmail&logoColor=white)](mailto:ssemamborodney94@gmail.com)
[![LinkedIn](https://img.shields.io/badge/LinkedIn-Connect-0A66C2?style=flat&logo=linkedin&logoColor=white)](www.linkedin.com/in/ssemambo-rodney-3b4b223b7) ← add your LinkedIn
[![Location](https://img.shields.io/badge/Location-Kampala%2C%20Uganda-green?style=flat&logo=google-maps&logoColor=white)]()

---

*Open to remote Laravel/PHP roles and freelance projects.*
